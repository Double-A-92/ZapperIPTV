...
object MediaSourceHelper {
    ...
    fun createDataSourceFactory(): DataSource.Factory {
        val defaultRequestProperties = mutableMapOf(
    ...
    private fun isHls(url: String): Boolean =
        url.contains(".m3u8") || url.contains("m3u8") || url.contains("/hls/") ||
                url.contains("stvp-") || url.contains("playlist") || url.contains(".m3u")

    private fun isDash(url: String): Boolean = url.contains(".mpd") || url.contains("dash")

    private fun isTs(url: String): Boolean =
        url.contains(".ts") || url.contains("format=ts") || url.contains("output=ts") ||
                url.contains("mpegts") || url.contains(":25461") || url.contains("/live/")
}
